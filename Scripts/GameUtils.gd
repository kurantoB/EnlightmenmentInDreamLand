...
static func path_intersects_border(path_from : Vector2, path_to : Vector2, border_pt_a : Vector2, border_pt_b : Vector2):
	...
	if path_from.x == path_to.x:
		...
		# if border is not vertical
			...
			return [intersect_y >= min(path_from.y, path_to.y) and intersect_y <= max(path_from.y, path_to.y), path_from.x, intersect_y]
	# else if border is vertical
		...
		return [intersect_y >= min(border_pt_a.y, border_pt_b.y) and intersect_y <= max(border_pt_a.y, border_pt_b.y), border_pt_a.x, intersect_y]
	# else if path and border are parallel
		...
		and intersect_x >= min(path_from.x, path_to.x) and intersect_x <= max(path_from.x, path_to.x)), intersect_x, path_m * intersect_x + path_b]
